...
const AiToolSelect = ({ text, selected, SetAiModeOption, option }) => {
    return (
      <div className={selected ? styles.AiToolsSelectDark : styles.AiToolsSelect} aria-hidden="true" onClick={() => SetAiModeOption(option)} onKeyDown={()=>SetAiModeOption(option)} >
      {text}
        </div>
    );
};
...
const AiInputField = ({ mode, SubmitAiSearchRequest }) => {
    ...
    useEffect(() => {
        if (mode == 3) {
            ...
            SetInpPlaceHolder("Write a letter email for ");
        } else if (mode == 4) {
            SetInpPlaceHolder("Calculate ");
        }else if (mode == 1) {
            SetInpPlaceHolder("Write an essay about ");
        }else if (mode == 5) {
